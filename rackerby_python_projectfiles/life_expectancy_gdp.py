...
plt.title("Average GDP Per Country between 2000 - 2015")
plt.show()
...
plt.title("Average Life Expectancy at Birth per Country between 2000 - 2015")
plt.show()
...
# Chile has one of highest life expectancy with one of lowest GDP
...
plt.figure(figsize=(12, 7)) 
sns.set_palette("Set3")
vplot = sns.violinplot(data=df, x="Country", y="LEABY")
plt.ylabel("Life expectancy at birth (years)")
plt.show()
fig = vplot.get_figure()
fig.savefig("leaby_vs_country_violinplot1.png")
...
# Which countries' bars changes the most? China + US GDP Changed the most, Zimbabwe's life expectation changed the most
# What years are there the biggest changes in the data? #
# Which country has had the least change in GDP over time? Zimbabwe. Chile's change was also quite low
# How do countries compare to one another? GDP: US is largest, followed by China, Germany, Mexico, Chile, and Zimbabwe.
#                                       Life expectancy: Chile and Germany highest followed by US, Mexico, and China, then Zimbabwe
# Now that you can see the both bar charts, what do you think about the relationship between GDP and life expectancy? Not really a corelation
# Can you think of any reasons that the data looks like this for particular countries? Healthcare, quality of living, professions
...
g.map(plt.scatter, "GDP", "LEABY", edgecolor="w")
g.set_axis_labels("GDP (USD)", "Life expectancy (years)")
g.set(xticks=[0, 0.5e13, 1e13, 1.5e13, 2e13])
g.add_legend()
plt.show()
g.savefig("leaby_vs_gdp_scattergraph2.png")
...
g3.map(plt.plot, "Year", "LEABY")
g3.set_axis_labels("Year", "Life expectancy at birth (years)")
g3.set(xticks=[2000, 2002, 2004, 2006, 2008, 2010, 2012, 2014, 2016])
g3.add_legend()
plt.show()
g3.savefig("leaby_vs_year_linegraph1.png")
...
#zimbabwe changed the most btwn 2006-2010
#mexico and germany have least change in life expectancy over time
#china - govt puts in legislation to move from developing to developed country
#zimbabwe - lower probably due to hiv/aids and subsequent treatment
#other countries are developed or almost-developed (mexico) and as such would have higher life expectancies
#ps sorry i cant get rid of the cell below :/
...
g4.map(plt.plot, "Year", "GDP")
g4.set_axis_labels("Year", "GDP (USD)")
g4.set(xticks=[2000, 2002, 2004, 2006, 2008, 2010, 2012, 2014, 2016])
g4.add_legend()
plt.show()
g4.savefig("gdp_vs_year_linegraph1.png")
...
# China and US = highest, Chile and Zimbabwe = lowest
...
# US, Chile, and Germany have highest; Zimbabwe have lowest
...
#the government made changes to import and export laws, as well as manufacturing laws.
#ie if another country wants to sell/manufacture in china they must hire chinese employees and they must disclose their
#technology to the government (ie fabrication, propriarity data, etc) so china can produce it themself.
